...
public:
    ...
    Report* buildShortReport(Builder* builder)
    {
        builder->setType();
        builder->setHeader();
...
int main()
{
    ...
    Report* fullReport = director->buildFullReport(new ExcelBuilder{ reportData });
    fullReport->show();

    Report* shortReport = director->buildShortReport(new PdfBuilder{ reportData });
    shortReport->show();
}
