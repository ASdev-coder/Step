...
template<class T>
List<T>& List<T>::operator+=(const List<T>& obj) // TODO: void? 
{
    ListItem<T>* iterator = obj.head;

    while (iterator != nullptr)
    {
        this->pushBack(iterator->getVal());         // TODO: optimize???
...
T List<T>::remove(uint id)
{
    ...
    ListItem<T>* prev = this->find(id - 1); // TODO: :-)
    ListItem<T>* next = this->find(id + 1); // TODO: :-)
...
void List<T>::insert(uint id, const T& val)
{
    ...
    ListItem<T>* prev = this->find(id - 1); // TODO: :-)
    ListItem<T>* next = this->find(id);     // TODO: :-)
